...
function InstructorDashboard() {
	...
	const fetchData = async () => {
		if (tokenData) {
			let url = `${baseUrl}/api/classes?instructor=${tokenData.account.id}`;
			let response = await fetch(url);
			if (response.ok) {
				let data = await response.json();
				setClasses(data);
			}
			url = `${baseUrl}/api/events/instructor/${tokenData.account.id}`;
			response = await fetch(url);
			if (response.ok) {
				let data = await response.json();
				setEvents(data);
			}
			url = `${baseUrl}/api/reservations/instructors/${tokenData.account.id}`;
			response = await fetch(url);
			if (response.ok) {
				let data = await response.json();
	...
	const changeStatus = async (e, id, enrolled) => {
		...
		let url = `${baseUrl}/api/reservations/${id}`;
		const fetchConfig = {
			method: 'put',
								...
								return (
									<div key={index}>
										<a
											...
											style={{ color: '#000000' }}
										>
											<h3 className="d-inline-flex align-text-middle mb-3">
