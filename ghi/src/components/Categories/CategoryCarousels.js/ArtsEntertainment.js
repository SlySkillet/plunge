...
function ArtCategory() {
	...
	useEffect(() => {
		async function loadArtClasses() {
			const response = await fetch('http://localhost:8000/classes?category=3');
			if (response.ok) {
	...
	return returnCategoryJSX(artClasses1, artClasses2, 'Arts & Entertainment');
}
