...
function SportsCategory() {
	...
	useEffect(() => {
		async function loadSportsClasses() {
			const response = await fetch('http://localhost:8000/classes?category=5');
			if (response.ok) {
	...
	return returnCategoryJSX(sportsClasses1, sportsClasses2, 'Sports & Gaming');
}
